...
export function RecurringTransactions() {
  ...
  const { data, addRecurringTransaction, totalRecurringExpenses, isLoaded } = useFinancialData()
  ...
  const handleAddTransaction = () => {
    if (newTransaction.name && newTransaction.amount && newTransaction.nextDate && newTransaction.category) {
      ...
      <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" className="bg-purple-600 hover:bg-purple-700" onClick={() => setShowAddForm(!showAddForm)}>
        ...
        <CardContent className="space-y-4">
          {showAddForm && (
              <div className="p-4 bg-slate-700/50 rounded-lg space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-white font-medium">Nouvelle transaction récurrente</h4>
                  <Button
                      size="sm"
                      variant="ghost"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                        ...
                        onChange={(e) => setNewTransaction((prev) => ({ ...prev, nextDate: e.target.value }))}
                        className="bg-slate-700 border-slate-600 text-white"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                      ...
                      onChange={(e) => setNewTransaction((prev) => ({ ...prev, logo: e.target.value }))}
                      placeholder="🎬 ou emoji"
                      className="bg-slate-700 border-slate-600 text-white"
                  />
                </div>

                <Button onClick={handleAddTransaction} className="w-full bg-purple-600 hover:bg-purple-700">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-slate-400 text-sm">{formatNextDate(transaction.nextDate)}</span>
                  </div>
                </div>
              </div>
          ))}
          ...
          {data.recurringTransactions.length === 0 && !showAddForm && (
              <div className="text-center py-8 text-slate-400">
                ...
                <p className="text-sm mt-1">Ajoutez vos abonnements et dépenses fixes</p>
              </div>
          )}
          ...
          {data.recurringTransactions.length > 0 && (
              <div className="pt-4 border-t border-slate-700">
                ...
                {totalRecurringExpenses.toLocaleString("fr-FR", { style: "currency", currency: "EUR" })}
              </span>
                </div>
              </div>
          )}
